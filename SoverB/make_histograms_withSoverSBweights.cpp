...
void make_histograms(TString config_name="config_for_gof_2016.cfg") {
  ...
  const TString directory_friendtree = cfg.get<string>("directory_friendtree");
  ...
  TString weight      = "xsec_lumi_weight*mcweight*puweight*effweight*friendtree.soversb*trigger_filter_weight*";
  ...
  for(auto &cat : category_map){
    cat.second.sample_list["0_Data"].weightString   = "1*friendtree.soversb*";
    cat.second.sample_list["0_Data"].weightStringSS = "1*friendtree.soversb*";
    cat.second.sample_list["1_QCD"].weightString    = "1*friendtree.soversb*";
    cat.second.sample_list["1_QCD"].weightStringSS  = "qcdweight*friendtree.soversb*";
    ...
    if(use_embedded){
      cat.second.sample_list["9_EMB"].weightString   = "mcweight*effweight*embeddedWeight*embedded_stitching_weight*embedded_rate_weight*friendtree.soversb*";
      cat.second.sample_list["9_EMB"].weightStringSS = "mcweight*effweight*embeddedWeight*embedded_stitching_weight*embedded_rate_weight*qcdweight*friendtree.soversb*";
  ...
  tree_->AddFriend("friendtree=TauCheck", directory_friendtree + "/" + Data.filename );
  ...
  for(auto & cat : category_map){
    ...
    for(auto & smpl : cat.second.sample_list){
      ...
      tree->AddFriend("friendtree=TauCheck", directory_friendtree + "/" + smpl.second.filename);

      //*******************************************************************************************
      // Calculate the binning
      if( smpl.second.name == "data_obs" ){
	if(!cat.second.plot_2d){
      cat.second.binning_1d = calc_binning_1d(take_percentile_subrange, apply_equidistant_binning, cat.second, directory, directory_friendtree);
      //cat.second.binning_1d = calc_binning_1d(take_percentile_subrange, apply_equidistant_binning, cat.second, directory);
   }
	else{
      std::pair<vector<float>, vector<float>> binning = calc_binning_2d(take_percentile_subrange, apply_equidistant_binning, cat.second, directory, directory_friendtree);
   ...
   tree->AddFriend("friendtree=TauCheck", directory_friendtree + "/" + sys.second.filename  );
