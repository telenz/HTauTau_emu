...
void PlotSys(
	     TString category = "em_vv",
	     TString histName = "VV",
	     TString sysName  = "CMS_scale_e_Run2016",
	     TString fileName = "output/2016/htt_em.inputs-sm-Run2016-ML.root",
	     float upRange = -100
	     ) {

  TString header = histName;
  ...
  std::cout << "  Up/Central = " <<  histUp->GetSumOfWeights()/histNominal->GetSumOfWeights() 
	    << "  Down/Central = " << histDown->GetSumOfWeights()/histNominal->GetSumOfWeights() << std::endl;

  int nBins = histNominal->GetNbinsX(); 

  histUp->GetXaxis()->SetRangeUser(10,900);
  histUp->SetLineWidth(2);
  histNominal->SetLineWidth(2);
  histDown->SetLineWidth(2);
  histUp->GetYaxis()->SetRangeUser(0.0,1.1*histUp->GetMaximum());
  ...
  histNominal->GetYaxis()->SetTitleSize(0.06);
  histUp->GetYaxis()->SetTitleSize(0.06);
  histDown->GetYaxis()->SetTitleSize(0.06);
  ...
  //  ratioCentral->SetFillColor(1);  //  ratioCentral->SetMarkerStyle(21);
  ...
  histDown->Draw("hsame");
  TLegend * leg = new TLegend(0.48,0.63,0.92,0.85);
  SetLegendStyle(leg);
  leg->SetHeader(header);
  leg->AddEntry(histNominal,sysName,"lp");
  ...
  canv1->Print("figures/"+histName+"_"+sysName+".png");
