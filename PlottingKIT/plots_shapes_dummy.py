...
def main(args):
    if args.gof_variable != None:
        ...
        if args.categories == "stxs_stage0":
            ...
            for channel in ["et", "mt", "tt", "em"]:
                channel_categories[channel] += ["1_A", "1_B", "1_C"]
        else:
        ...
        for category in channel_categories[channel]:
            ...
            for i in plot_idx_to_add_signal:
                if category=="1" or category=="2":
                   ...
                   dummy = rootfile.get(era, channel, category, "EMB")
                   dummy.Reset()
                   plot.subplot(i).add_hist(dummy, "ggH")
                   plot.subplot(i).add_hist(dummy, "ggH_top")
                   plot.subplot(i).add_hist(dummy, "qqH")
                   plot.subplot(i).add_hist(dummy, "qqH_top")        
